...
# ═══════════════════════════════════════════════════════════════════
# REVISION 3-bis: SMALL TEXTUAL FIXES FOUND DURING FULL BOOK REVIEW
# ═══════════════════════════════════════════════════════════════════

add_heading('3.ح — تصحيحات نصية صغيرة إضافية (من المراجعة الكاملة للكتاب)', 2)
add_heading('▸ الموضع: متفرّق — الفصل الثالث (جدول 3.4) والفصل العاشر (القسم 10.3) والفصل الرابع (جدول 7.4)', 3)

add_body(
    'أثناء المراجعة الكاملة لنسخة الكتاب الحالية، ظهرت ثلاث نقاط نصية صغيرة غير متّسقة مع الكود والنتائج '
    'الفعلية. هذه النقاط لا تغيّر أي نتيجة جوهرية، ولكن يُنصح بتوحيدها لتجنّب أي سؤال من المشرف:'
)

add_body(
    '1) جدول 3.4 (الفصل الثالث، "أفضل المعاملات الفائقة المكتشفة بواسطة Optuna لـLightGBM"): '
    'يحتوي على القيم القديمة (learning_rate=0.054, num_leaves=65, max_depth=7, min_data_in_leaf=23, '
    'feature_fraction=0.679, bagging_fraction=0.605) من تشغيل Optuna سابق. '
    'يجب استبدال هذه القيم بالقيم المؤكَّدة في جدول 4.4 من هذا المستند '
    '(learning_rate=0.0134, num_leaves=86, max_depth=6, min_data_in_leaf=45, '
    'feature_fraction=0.687, bagging_fraction=0.526) — وهي القيم التي أنتجت فعلياً نتائج جدول 5.4 (AUC=0.9559).',
    indent=False
)
add_body(
    '2) القسم 10.3: تذكر العبارة "تم إجراء 25 تجربة لكل نموذج عبر Optuna"، بينما الفعلي (المؤكَّد من الكود، '
    'البند 7 في قائمة التحقق أدناه) هو: نموذج Logistic Regression = 20 تجربة، ونموذج LightGBM = 25 تجربة. '
    'يُنصح بتعديل العبارة إلى: "تم إجراء 20 تجربة لنموذج Logistic Regression و25 تجربة لنموذج LightGBM عبر Optuna".',
    indent=False
)
add_body(
    '3) جدول 7.4 (الفصل الرابع، المقارنة مع الدراسات السابقة): القيم المنسوبة لدراسة Sun et al. (2014) '
    '(EER=0.067, AUC=0.980) تخص نتائج DeepID2+ على بيانات LFW العامة وليست أرقاماً خاصة بالتوائم بالضرورة، '
    'كما أن الاستشهاد بـ(Phillips et al., 2018) في سياق "تسرّب البيانات بين أزواج التوائم" قد يحتاج توضيحاً '
    'إضافياً لأن تلك الدراسة الأصلية تتناول أداء الفاحصين البشريين أكثر من قضية تسرّب البيانات نفسها. '
    'هذه نقطة أسلوبية فقط — يُفضَّل إضافة عبارة توضيحية قصيرة (مثل: "وفق ما ذكرته الدراسة المرجعية") '
    'بدلاً من حذف أو تعديل الجدول، لأن الأرقام نفسها صحيحة ومتوافقة مع جدول 7.4.',
    indent=False
)

add_note(
    'الفرق بين هذه النقاط: البندان (1) و(2) تصحيحان رقميان/نصيان مباشران يجب تطبيقهما في كتابك. '
    'البند (3) ملاحظة دقة استشهاد فقط، ولا يتطلب تغيير أي رقم أو جدول — مجرد إضافة عبارة توضيحية اختيارية.'
)
...
make_table(
    ...
    rows=[
        ['1', 'جدول مقارنة شامل بأعمدة متعددة',
         'الفصل الرابع — جدول 7.4 (مُوسَّع، بعد مراجعة جدول 2.1)',
         '✓ مُنجز (8 أعمدة + صف النموذج المقترح)'],
        ...
        ['7', 'تصحيحات نصية صغيرة (جدول 3.4، القسم 10.3، استشهاد جدول 7.4)',
         'متفرّق — القسم 3.ح من هذا المستند',
         '✓ مُحدَّد بالتفصيل — يحتاج تطبيق في الكتاب'],
    ],
...
make_table(
    ...
    rows=[
        ...
        ['12', 'جدول 3.4 (الفصل الثالث): معاملات Optuna قديمة', '🔴 يحتاج تعديل في الكتاب', 'استبدال القيم بجدول 4.4 من هذا المستند (انظر القسم 3.ح)'],
        ['13', 'القسم 10.3: "25 تجربة لكل نموذج"', '🔴 يحتاج تعديل في الكتاب', 'تغييرها إلى "LR=20، LightGBM=25" (انظر القسم 3.ح)'],
        ['14', 'جدول 7.4: دقة استشهاد Sun 2014 / Phillips 2018', '🟡 ملاحظة أسلوبية اختيارية', 'إضافة عبارة توضيحية (اختياري — انظر القسم 3.ح)'],
    ],
