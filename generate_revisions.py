...
add_body(
    'يُقدِّم الجدول التالي مقارنةً شاملةً بين الدراسات السابقة الواردة في جدول 7.4 من الفصل الرابع '
...
make_table(
    ...
    caption_text='جدول 2.1: مقارنة شاملة بين الدراسات السابقة في مجال التعرف على وجوه التوائم المتماثلة (وفق جدول 7.4)',
    highlight_last=True
)
...
add_note(
    '★ ملاحظة للتحقق: أرقام الصور لكل طيّة (3,797 / 946 / ... إلخ) قيم تقديرية منطقية '
    'لتوضيح التوازن النسبي بين الطيّات، ولكنها لم تُؤخذ من ناتج الكود الفعلي. '
    'يُرجى استخراج الأرقام الحقيقية من ناتج GroupKFold في Colab (مثلاً عبر طباعة '
    'len(train_idx) وlen(test_idx) لكل طيّة) واستبدال هذه القيم بها قبل التسليم.'
)
...
make_table(
    ...
    rows=[
        ...
        ['المتجه الكامل لكل زوج',          '—',                            '—',                          '—',                                      '1,030 بُعداً\n(يُرجى التحقق\nمن الكود)'],
    ],
    caption_text='جدول 3.4 (جديد): تفصيل مكوّنات متجه الزوج المُستخدَم في التصنيف'
)

add_note(
    '★ ملاحظة: إجمالي الأبعاد المُدرَج في هذا الجدول وفق المكوّنات المذكورة هو 1,027 بُعداً تقريباً، '
    'بينما القيمة الموثَّقة في عمل سابق هي 1,030. يُرجى مراجعة كود استخراج السمات لتحديد '
    'المكوّنات الإضافية الدقيقة (قد تتضمن سمات ملمس إضافية أو مسافات أخرى) قبل اعتماد '
    'القيمة النهائية في الرسالة.'
)
...
make_table(
    ...
    rows=[
        ...
        ['النموذج المقترح (LightGBM)',    '2026', '0.956', '0.122', 'ND-TWINS 2009-2010', '— (مرجع)'],
    ],
...
add_caption('الشكل 3.4 (أوبتونا): ناتج تحسين Optuna — أفضل EER محقق (يُرجى ذكر عدد التجارب الفعلي من الكود)')
...
make_table(
    ...
    rows=[
        ...
        ['7', 'عدد تجارب Optuna (100 أم 20-25؟)', '🟡 يحتاج تأكيد', 'حددي العدد الفعلي من سجل Colab'],
        ['8', 'أبعاد متجه الزوج (1,027 من المكوّنات أم 1,030؟)', '🟡 يحتاج تأكيد', 'افحصي كود بناء متجه الزوج'],
        ['9', 'عدد صور كل طيّة (3,797/946 ...)', '🟡 تقديري', 'استخرجي len(train)/len(test) الفعلية من GroupKFold'],
